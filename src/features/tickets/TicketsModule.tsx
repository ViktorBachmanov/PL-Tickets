...
export default function TicketsModule(props: Props) {
  ...
  const tikckets = props.tickets.map((ticket: TicketCardData) => (
    <TicketCard key={ticket.id} ticket={ticket} setCurrentTicketById={props.setCurrentTicketById} />
  ));

  return (
    <Grid container columns={totalColumns} spacing={1} style={{ padding: '30px' }}>
      {tikckets}
    </Grid>
  );
}
